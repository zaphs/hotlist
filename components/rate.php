 ...
 * @package ow.plugins.hotphoto.components
 * @since 1.0
 */
class HOTPHOTO_CMP_Rate extends OW_Component
{
    ...
    public function __construct( $pluginKey, $entityType, $entityId, $ownerId, $sex )
    {
        ...
        OW::getDocument()->addScript(OW::getPluginManager()->getPlugin('hotphoto')->getStaticJsUrl() . 'hotphotorate.js');
        ...
        $jsParamsArray = array(
            ...
            'ownerId' => $ownerId,
            'nextPhotoUrl' => OW::getRouter()->urlFor('HOTPHOTO_CTRL_Rate', 'getNextPhoto'),
            'refreshPhotoUrl' => OW::getRouter()->urlFor('HOTPHOTO_CTRL_Rate', 'refreshPhoto'),
            'sex' => $sex
        );

        OW::getDocument()->addOnloadScript("var rate$cmpId = new HotphotoRate(" . json_encode($jsParamsArray) . "); rate$cmpId.init();");
    }
}